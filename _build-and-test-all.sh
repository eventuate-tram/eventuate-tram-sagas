...
if [[ "${SPRING_PROFILES_ACTIVE}" != "ActiveMQ" ]]; then
    ...
    ./gradlew -x orders-and-customers-micronaut-integration-tests:test -x :orders-and-customers-micronaut:test -x :eventuate-tram-sagas-common-micronaut:test :orders-and-customers-spring:cleanTest build
fi
