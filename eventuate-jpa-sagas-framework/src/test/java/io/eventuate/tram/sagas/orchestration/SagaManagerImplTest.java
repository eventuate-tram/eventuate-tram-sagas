...
public class SagaManagerImplTest {
    ...
    public void setUp() {
        ...
        final SimpleSagaRegistrar sagaRegistrar = new SimpleSagaRegistrar(Collections.singletonList(testSaga));

        final SagaOrchestrator sagaOrchestrator = new SagaOrchestrator(sagaRegistrar, channelMapping, messageConsumer, sagaLockManager,
                commandProducer, sagaCommandProducer, sagaInstanceRepository);

        sm = new SagaManagerImpl<>(testSaga, sagaOrchestrator);
    ...
    private void startSaga() {
        when(sagaDefinition.getActions(initialSagaData)).thenReturn(makeFirstSagaActions());
    ...
    private void handleReply(boolean compensating) {
        ...
        when(sagaDefinition.getReplyActions(anyString(), any(TestSagaData.class), any(Message.class)))
